    ...
    func exportVideo(start: Date,end: Date,speed: Double,layoutOverride: String,deviceID: String?,includeMouse: Bool,heatMode: String,sound: String = "keyboard",targetDuration: Double? = nil) {
        ...
        DispatchQueue.global(qos:.userInitiated).async { [weak self] in
            do {
                ...
                let cache=fm.homeDirectoryForCurrentUser.appendingPathComponent("Library/Caches/KeyTrace/VideoJobs/"+id)
                ...
                DispatchQueue.main.async {
                    ...
                    do {
                        ...
                        self.videoTask=task;self.videoCache=cache
                        ...
                        task.terminationHandler={ [weak self] process in
                            ...
                            DispatchQueue.main.async {
                                guard let self else {return}
                                self.videoProgressTimer?.invalidate();self.videoProgressTimer=nil;self.videoTask=nil;self.videoCache=nil;self.exporting=false
                    ...
                    } catch {self.videoProgressTimer?.invalidate();self.videoTask=nil;self.videoCache=nil;self.exporting=false;self.exportStatus=M("导出失败：\(error.localizedDescription)", "Export failed: \(error.localizedDescription)");try? fm.removeItem(at:cache)}
                }
